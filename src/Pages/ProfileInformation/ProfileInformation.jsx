...
const ProfileInformation = () => {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(
      `https://collage-nest-server.vercel.app/myCollage?email=${user?.email}`
    )
      .then((res) => res.json())
      .then((data) => {
        setMyCollages(data);
        ...
        fetch(`https://collage-nest-server.vercel.app/collage/${id}`, {
          ...
          .then((data) => {
            ...
            if (data.deletedCount > 0) {
              ...
              const remaining = myCollages.filter(
                (collage) => collage._id !== id
              );
  ...
  return (
    <div className="w-full md:w-2/3 lg:w-1/3 mx-auto border-2 border-blue-400 p-4 mt-8 ">
      ...
      {myCollages.length < 1 ? (
        <h1 className="text-xl font-semibold">You Did Not Apply Any Collage</h1>
      ) : (
        <div>
          {myCollages.map((collage) => (
            <li className="flex gap-4 mb-4 items-center" key={collage._id}>
              {collage.collageName}{" "}
              <span
